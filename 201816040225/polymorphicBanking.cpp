...
int main()
{
  ...
  vector<Account *> accounts(4);
   ...
   accounts[i]->debit(withdrawalAmount);//debit function
      ...
      accounts[i]->credit(depositAmount);//credit function
      // downcast pointer


       SavingsAccount *savingsAccountPtr =
      dynamic_cast<SavingsAccount *>(accounts[i]);//base class pointer to derived class pointer
      ...
      if ( savingsAccountPtr!=nullptr/* Write a test to determine if savingsAccountPtr isn't 0 */ )
      {
         double interestEarned = savingsAccountPtr->calculateInterest()/* Call member function calculateInterest//calculate the interestEarned
								    through savingsAccountPtr */;
         cout << "Adding $" << interestEarned << " interest to Account "
            << i + 1 << " (a SavingsAccount)" << endl;
            savingsAccountPtr->credit(interestEarned);//credit function
      ...
      cout << "Updated Account " << i + 1 << " balance: $"
         << accounts[i]->getBalance()//get the finally balance/* Call the getBalance function through Account pointer i */
		 << "\n\n";
