...
int main()
{
   ...
   for ( size_t i = 0; i < accounts.size(); i++ )
   {
      ...
      SavingsAccount *savingsAccountPtr =dynamic_cast <SavingsAccount *>(accounts[i]);
         /*编写动态强制转换操作以尝试向下转换指向SavingsAccount指针的帐户指针i*/
      ...
      {
         double interestEarned = savingsAccountPtr->calculateInterest();/*通过savingsAccountPtr调用成员函数calculateInterest；*/;
         cout << "Adding $" << interestEarned << " interest to Account "
            << i + 1 << " (a SavingsAccount)" << endl;
       			savingsAccountPtr->credit(interestEarned);	/*使用credit函数将利息记入savingsAccountPtr所指的SavingsAccount   
