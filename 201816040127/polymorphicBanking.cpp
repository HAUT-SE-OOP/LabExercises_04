...
int main()
{
   ...
   // declarations for a vector of four pointers to Account objects, called accounts
   ...
   for ( size_t i = 0; i < accounts.size(); i++ )
   {
      ...
      SavingsAccount* savingsAccountPtr = dynamic_cast<SavingsAccount*>(accounts[i]);
         //  a dynamic_cast operation to to attempt to downcast Account pointer i to a SavingsAccount pointer

      // if Account is a SavingsAccount, calculate and add interest
      if (savingsAccountPtr ) // a test to determine if savingsAccountPtr isn't 0
      {
       double interestEarned = savingsAccountPtr->calculateInterest();// Call member function calculateInterest through savingsAccountPtr
