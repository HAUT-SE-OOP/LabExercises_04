...
int main()
{
   ...
   for ( size_t i = 0; i < accounts.size(); i++ )
   {
      cout << "Account " << i + 1 << " balance: $" 
       << accounts[i]->getBalance()<<endl/* Call the getBalance function through Account pointer i */;
    ...
    accounts[i]->debit(withdrawalAmount);
      ...
      if (nullptr!=savingsAccountPtr /* Write a test to determine if savingsAccountPtr isn't 0 */ )
      {
