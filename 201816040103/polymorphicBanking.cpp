...
int main()
{
   ...
   vector <Account *> accounts(4);/* Write declarations for a vector of four pointers
   ...
   {
	  cout << "Account " << i + 1 << " balance: $" <<accounts[i]->getBalance();/* Call the getBalance function through Account pointer i */
      ...
      {
         double interestEarned = savingsAccountPtr->calculateInterest() /* Call member function 
		 calculateInterest through savingsAccountPtr */;
         cout << "Adding $" << interestEarned << " interest to Account "
            << i + 1 << " (a SavingsAccount)" << endl;
         savingsAccountPtr->credit(withdrawalAmount);/* Use the credit function to credit interestEarned to
