...
int main()
{
   ...
   for ( size_t i = 0; i < accounts.size(); i++ )
   {
     ...
     Account *savingsAccountPtr = accounts[ i ];
      ...
      if ( /* Write a test to determine if savingsAccountPtr isn't 0 */
          i == 0 || i == 2 )
      {
          //cout << "this is nowBalance ****"<<savingsAccountPtr->getBalance()<<endl;
         double interestEarned = /* Call member function calculateInterest
								    through savingsAccountPtr */
                                    savingsAccountPtr->getBalance()*savingsAccountPtr->calculateInterest();
