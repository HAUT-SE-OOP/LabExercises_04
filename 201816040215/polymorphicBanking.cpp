...
int main()
{
   ...
   for ( size_t i = 0; i < accounts.size(); i++ )
   {
      cout << "Account " << i + 1 << " balance: $"
         << i->getbalance();/* Call the getBalance function through Account pointer i */;

      double withdrawalAmount = 0.0;
      cout << "\nEnter an amount to withdraw from Account " << i + 1
         << ": ";
      cin >> withdrawalAmount;
      i->debit();/* Call the debit function through Account pointer i */

      double depositAmount = 0.0;
      cout << "Enter an amount to deposit into Account " << i + 1
         << ": ";
      cin >> depositAmount;
      i->credit();/* Call the credit function through Account pointer i */

      // downcast pointer
      SavingsAccount *savingsAccountPtr =
         *i = &savingsaccount;/* Write a dynamic_cast operation to to attempt to downcast
		    Account pointer i to a SavingsAccount pointer */

      // if Account is a SavingsAccount, calculate and add interest
      if ( savingAccountPtr!-> 0 /* Write a test to determine if savingsAccountPtr isn't 0 */ )
      {
         ...
         savingsAccountPtr -> credit();/* Use the credit function to credit interestEarned to
